      ...
      return (
        <ReactMarkdown
          components={{
            ...
            // eslint-disable-next-line @typescript-eslint/no-unused-vars
            h4: ({ node, ...props }) => (
              <h4 {...props} className="text-sm font-bold mt-2 mb-1" />
            ),
            // eslint-disable-next-line @typescript-eslint/no-unused-vars
            ul: ({ node, ...props }) => (
            ...
            code: ({ inline, className, children, ...props }: any) =>
              inline ? (
                <code
                  {...props}
                  className="px-1 py-0.5 bg-tech-navy font-mono text-sm rounded"
                >
                  {children}
                </code>
              ) : (
                <code
                  {...props}
                  className="block p-2 bg-tech-navy font-mono text-sm rounded my-2 whitespace-pre-wrap"
                >
                  {children}
                </code>
              ),
            ...
            blockquote: ({ node, ...props }) => (
              <blockquote
                {...props}
                className="border-l-4 border-tech-teal pl-4 italic my-2"
              />
            ),
            // eslint-disable-next-line @typescript-eslint/no-unused-vars
            strong: ({ node, ...props }) => (
      ...
      {isOpen && (
        ...
        >
          <div className="px-4 py-3 bg-tech-navy border-b border-tech-light flex justify-between items-center remove-scollbar">
            ...
            <button
              className="bg-transparent border-none text-[#a8a8c8] cursor-pointer flex items-center justify-center p-1 rounded-full hover:bg-[#342b55] hover:text-white transition-colors"
          ...
          <div className="flex-1 remove-scollbar overflow-y-auto p-4 flex flex-col gap-4 bg-tech-navy scrollbar-thin scrollbar-thumb-[#342b55] scrollbar-track-transparent scrollbar-thumb-rounded">
            {messages.length === 0 && (
              <div className="flex items-start gap-3 bg-tech-navy rounded-lg p-4 shadow max-w-[85%] mx-auto border border-tech-light">
                ...
                <div>
                  <h3 className="m-0 mb-2 text-[15px] font-semibold text-white">
                    Hi, I&apos;m David&apos;s Assistant
                  </h3>
                  <p className="m-0 text-sm text-[#d1d1e6]">
                    👋 Hello! I can tell you about David&apos;s skills,
                    experience, projects, and qualifications. How can I help you
                    today?
                  </p>
                </div>
              </div>
            ...
            {messages.map((msg, index) => (
              <div
                key={index}
                className={`flex gap-2 max-w-[90%] ${msg.type === "user" ? "flex-row-reverse self-end" : "self-start"}`}
              >
                {msg.type === "bot" && (
                ...
                <div className="flex flex-col gap-0.5">
                  <div
                    className={`p-2 px-3 rounded-2xl remove-scollbar border border-tech-light min-w-[40px] max-w-full relative break-words ${
                      msg.type === "user"
                  ...
                  <div
                    className={`flex items-center gap-1 text-xs ${msg.type === "user" ? "justify-end" : ""}`}
                  >
